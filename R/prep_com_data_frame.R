...
ensure_cip_code_has_period <- function(x) {
  gsub(pattern = "(^[0-9]{1,2}$)", replacement = "\\1\\.0000", x)
}

cip_code_has_a_period <- function(x) {
  grepl(x, pattern = "\\.")
}
...
add_leading_zero_to_cip_lefthand_side <- function(x) {
  dplyr::case_when(
